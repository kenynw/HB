...
public class PagerSlidingTabStrip extends HorizontalScrollView {
    ...
    public PagerSlidingTabStrip(Context context, AttributeSet attrs, int defStyle) {
        ...
        underlineMargin = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, underlineMargin, dm);
        ...
        underlineMargin = a.getDimensionPixelSize(R.styleable.AdvancedPagerSlidingTabStrip_apTabUnderlineMargin, underlineMargin);
        a.recycle();
    ...
    public void notifyDataSetChanged() {
        ...
        for (int i = 0; i < tabCount; i++) {
            ...
            if (pager.getAdapter() instanceof TabProvider) {
                View view = ((TabProvider) pager.getAdapter()).getSelectTabView(i, getSelectTabView(i));
...
//        updateTabStyles();
    ...
    private void drawTabNormalMode(Canvas canvas) {
        ...
        //绘制提示下划线
        if (isFollowText) {
            TextView textView = findTextViewFromTab(currentTab);
            if (textView != null) {
                final TextPaint paint = textView.getPaint();
                final float textWidth = paint.measureText(textView.getText().toString());
                rectPaint.setStrokeCap(Paint.Cap.ROUND);
                final float left = lineLeft + (currentTab.getWidth() - textWidth) / 2;
                canvas.drawRect(left, height - indicatorHeight, left + textWidth, height, rectPaint);
            }
        } else {
            canvas.drawRect(lineLeft + underlineMargin, height - indicatorHeight, lineRight - underlineMargin, height, rectPaint);
        }
        ...
        canvas.drawRect(underlineMargin, height - underlineHeight, tabsContainer.getWidth() - underlineMargin, height, rectPaint);
        // 绘制分割线
     ...
     * 设置指示器的宽度是否跟随文本
     */
    public void setFollowText(boolean followText) {
        isFollowText = followText;
    }

    @Nullable
    private TextView findTextViewFromTab(View view) {
        ...
        if (view instanceof ViewGroup) {
            ...
            for (int i = 0; i < group.getChildCount(); i++) {
                findTextViewFromTab(group.getChildAt(i));
            }
        }
        return null;
    }
    ...
    public void setSelectItem(int position) {
        ...
        for (int i = 0; i < tabsContainer.getChildCount(); i++) {
            ...
            if (i == position) {
                view = ((TabProvider) pager.getAdapter()).getSelectTabView(i, getSelectTabView(i));
                setSelectTabView(i, view);

            } else {
                view = ((TabProvider) pager.getAdapter()).getDisSelectTabView(i, getDisSelectTabView(i));
    ...
    public interface TabProvider {
        View getSelectTabView(int position, View convertView);

        View getDisSelectTabView(int position, View convertView);
    }
